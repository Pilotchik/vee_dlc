...
<div id="main">
	...
	{
		?>
		<h3>Группы <?= $type_r_name ?></h3>
		<?php
		for($i = 1;$i < 5;$i++)
		{
			?> 
			<h4><a href="<?= base_url() ?>rating/courses/<?= $i ?>"><?= $i ?> курс</a></h4> 
			<div class="row">
				<?php
				foreach($groups[$i] as $key)
				{
					?>
					<div class="col-xs-6 col-sm-3" style="text-align:center;">
						<h4><b><a href="<?= base_url() ?>rating/groups/<?= $key['id'] ?>/<?= $key['name_numb'] ?>" style="text-decoration:none;color:black;"><?= $key['name_numb'] ?></a></b></h4>
					</div>
					<?php
				}
				?>
