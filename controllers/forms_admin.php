...
class Forms_admin extends CI_Controller {
	...
	{
		$data['all_type_reg']=$this->captcha_model->getAllTypeReg();
		$data['all_forms']=$this->forms_model->getAllForms(1);
		foreach ($data['all_forms'] as $key)
		{
			$data['type_r'][$key['id']]=$this->captcha_model->getTypeReg($key['type_r']);
			$data['author'][$key['id']]=$this->captcha_model->getUserName($key['author_id']);
		}
		$data['error'] = $error;
	...
	function quest_view($error = "")
	{
		...
		$data['form_quests']=$this->forms_model->getAllQuests($form_id);
		$data['error'] = $error;
		$this->load->view('forms/forms_quest_view',$data);
	}
	...
	function quest_create()
	{
		...
		{
			$this->forms_model->createQuest($form_id);
			$error = "Вопрос создан";
		}
		$this->quest_view($error);
	}
