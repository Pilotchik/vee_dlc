...
class Forms extends CI_Controller {
	...
	//Функция отображения страницы со списков активных и архивных опросов
	function index()
	{
		$data['error'] = "";
		...
		foreach ($data['open_forms'] as $key)
		{
			...
			if ($key['type_r'] == 1)
			{
				...
				$data['kurs_status'][$key['id']]['abs'][4] = $data['count_resp'][$key['id']] - $summ_resp;
				$summ_resp += $data['kurs_status'][$key['id']]['abs'][4];
				for ($i = 1; $i < 4; $i++)
				{
	...
	//Функция для формирования интерфейса прохождения нелинейных опросов
	function play_form()
	{
		...
		if ($this->forms_model->checkFormId($form_id) > 0)
		{
			...
			$data['form_desc'] = $this->forms_model->getFormDesc($form_id);
	...
	//Функция асинхронного сохранения результатов опроса
	function autosave()
	{
		...
		$this->form_validation->set_rules('val3', 'Значение2', 'trim|xss_clean|is_natural|less_than[11]');
	...
	//Функция асинхронного удаления результатов опроса
	function autosave2()
	{
		//Проверка введённых данных
		$this->form_validation->set_rules('id_q', 'ID', 'trim|required|xss_clean|is_natural_no_zero');
	...
	//Функция формирования интерфейса пользователя по окончанию анкетирования
	function form_itog()
	{
	...
	//Функция асинхронной подгрузки результатов анкетирования
	function loadgraph()
	{
